...
export default function BookSearchModal({ visible, onClose, onBookSelect }: BookSearchModalProps) {
  ...
  useEffect(() => {
    const searchBooks = async () => {
      ...
      setLoading(true);
      const searchResults = await bookService.searchBook(debouncedQuery, 10);
  ...
  const renderResult = ({ item }: { item: BookMetadata }) => (
    <TouchableOpacity style={styles.resultItem} onPress={() => handleSelect(item)}>
      ...
      <View style={styles.resultInfo}>
        ...
        <Text style={styles.resultAuthor}>{item.authors.join(', ')}</Text>
      </View>
    </TouchableOpacity>
  );
    ...
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
      ...
      >
        <CozyCard style={styles.modalContent}>
          ...
          <Text style={styles.modalTitle}>Rechercher un livre</Text>
          <TextInput
            style={styles.input}
