...
// Importez les modales de recherche
...
export default function EditProfileScreen() {
  ...
  // Correction: Déclaration des états manquants
  ...
  const [currentAppLanguage, setCurrentAppLanguage] = useState<'fr' | 'en'>('en');

  useEffect(() => {
    const fetchCurrentLanguage = async () => {
      const locales = await Localization.getLocales();
      const lang = locales[0]?.languageCode === 'fr' ? 'fr' : 'en';
  ...
  useEffect(() => {
    const loadProfile = async () => {
      if (params.profileId) {
        ...
        setProfile({ name: t('manageProfiles.form.newProfileDefaultName'), avatarUri: null, films: [], books: [], tvShows: [] });
      }
      setIsLoading(false);
    };
    loadProfile();
  ...
  const handleSave = async () => {
    if (profile && profile.name && profile.name.trim()) {
      ...
      Alert.alert(t('common.oops'), t('manageProfiles.errors.nameRequired'));
    }
  };
  ...
  if (isLoading || !profile) { // Utilisation des états déclarés
    return (
      <BackgroundWrapper>
    ...
    <BackgroundWrapper>
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        <View style={[styles.header, { paddingTop: insets.top }]}>
          ...
          <Text style={styles.headerTitle}>{params.profileId ? t('manageProfiles.form.editTitle') : t('manageProfiles.form.newTitle')}</Text>
        ...
        <ScrollView contentContainerStyle={styles.scrollContent}>
            ...
            <Animated.View sharedTransitionTag={params.profileId ? `profile-${params.profileId}-avatar` : undefined}>
                ...
                {profile.avatarUri ? (
                    ...
                    <View style={styles.avatarPlaceholder}>
                      ...
                      <Text style={styles.avatarPlaceholderText}>{t('manageProfiles.form.choosePhoto')}</Text>
                    </View>
                )}
                </TouchableOpacity>
            </Animated.View>
          ...
          >
            <TextInput
              style={styles.input}
              placeholder={t('manageProfiles.form.namePlaceholder')}
          ...
          >
            <Text style={styles.sectionTitle}>{t('profile.categories.films')}</Text>
          ...
          >
            <Text style={styles.sectionTitle}>{t('profile.categories.books')}</Text>
             <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.itemsGrid}>
          ...
          >
            <Text style={styles.sectionTitle}>{t('profile.categories.tvShows')}</Text>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.itemsGrid}>
      ...
      {/* Les modales de recherche doivent aussi être traduites */}
      {/* Passer la langue actuelle de l'application aux modales */}
      <FilmSearchModal visible={searchModalType === 'film'} onClose={() => setSearchModalType(null)} onFilmSelect={handleAddFilm} userLanguage={currentAppLanguage} />
      <BookSearchModal visible={searchModalType === 'book'} onClose={() => setSearchModalType(null)} onBookSelect={handleAddBook} userLanguage={currentAppLanguage} />
      <TvShowSearchModal visible={searchModalType === 'tvShow'} onClose={() => setSearchModalType(null)} onTvShowSelect={handleAddTvShow} userLanguage={currentAppLanguage} />
    </BackgroundWrapper>
  );
}
